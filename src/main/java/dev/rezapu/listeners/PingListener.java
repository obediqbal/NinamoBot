...
public class PingListener extends BaseListener {
    public PingListener(){
        ...
        CommandsUtil.addCommand(pingCommand, true, true);
    }
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event){
        try{
            if (event.getMessage().getContentRaw().equals(".ping")){
                PingCommand pingCommand = CommandsUtil.getCommand(PingCommand.class);
                if(pingCommand!=null) pingCommand.action(event);
            }
        }catch (Exception e){
